---
page_title: "keycloak_oauth_identity_provider Resource"
---

# keycloak\_oauth\_identity\_provider Resource

Allows for creating and managing OAuth2 Identity Providers within Keycloak.

OAuth2 identity providers allow users to authenticate through a third party system using the OAuth2 protocol.

> **NOTICE:** This resource supports [write-only arguments](https://developer.hashicorp.com/terraform/language/resources/ephemeral#write-only-arguments)
> for client secret via the arguments `client_secret_wo` and `client_secret_wo_version`. Using write-only arguments
...
>
> For backward compatibility with provider behavior, the original `client_secret` argument remains available.
...
resource "keycloak_oauth_identity_provider" "realm_identity_provider" {
  ...
  user_info_url     = "https://userinfo.com"
...
ephemeral "random_password" "oauth_client_secret" {
  length           = 16
  override_special = "!#$%&*()-_=+[]{}<>:?"
}
...
resource "keycloak_oauth_identity_provider" "realm_identity_provider" {
  ...
  authorization_url        = "https://authorizationurl.com"
  client_id                = "clientID"
  client_secret_wo         = ephemeral.random_password.oauth_client_secret.result
...
- `client_secret_wo` - (Optional, Write-Only) The client secret. This is a write-only argument and Terraform does not store it in state or plan files. If omitted, this will fallback to use `client_secret`.
...
- `provider_id` - (Optional) The ID of the identity provider to use. Defaults to `oauth2`, which should be used unless you have extended Keycloak and provided your own implementation.
...
- `default_scopes` - (Optional) The scopes to be sent when asking for authorization. It can be a space-separated list of scopes.
...
- `extra_config` - (Optional) A map of key/value pairs to add extra configuration to this identity provider. This can be used for custom OAuth2 provider implementations, or to add configuration that is not yet supported by this Terraform provider. Use this attribute at your own risk, as custom attributes may conflict with top-level configuration attributes in future provider updates.
    - `clientAuthMethod` (Optional) The client authentication method.
    - `tokenIntrospectionUrl` (Optional) The token introspection endpoint URL.
    - `pkceEnabled` (Optional) Indicates whether PKCE is enabled.
    - `pkceMethod` (Optional) The PKCE method, for example `S256`.
    - `requiresShortStateParameter` (Optional) Indicates whether Keycloak should use a short state parameter.
...
```bash
$ terraform import keycloak_oauth_identity_provider.realm_identity_provider my-realm/my-idp
```
